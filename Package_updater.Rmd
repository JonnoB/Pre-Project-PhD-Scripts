...
files_to_convert <- list.files("/home/jonno/PowerGridNetworking/R", full.names = T) 
...
all_funcs_in_package <- tibble(class = "name",
  function_name = ls("package:PowerGridNetworking"),
  type = "functions",
  value = ls("package:PowerGridNetworking")
       )
...
list.files("/home/jonno/Spring_Embeddings_Paper", full.names = T, pattern = ".R") %>%
  walk(~{
...
This chunk uses the DependenciesGraphs package. It allows an interactive mapping od the network created by the function dependencies within a pacakge
```{r}
